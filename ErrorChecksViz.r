...
cat(sprintf("\\section{Phase 3}"), "\nHere are the results of Phase 3", "\n")


#length(na.omit(exportFile$ancvotetotal[exportFile$e1a_inventivized %in% c("no", "yes")])

exportFile$vcBin <- 0
exportFile$vcBin[!is.na(exportFile$votingexperience_complete)] <- 1

monitors <- exportFile[exportFile$e1a_inventivized %in% c("no", "yes"),]

veQu <- grep("^votingexperience_question_", names(exportFile), value=TRUE)
veQu <- c(veQu[1], veQu[3:length(veQu)])
for(vq in 1:length(veQu)){
   print(
       xtable(
             crosstab(monitors[, (veQu[vq])], monitors$e1a_inventivized,
                                           chisq=TRUE, prop.c=TRUE, prop.r=TRUE, plot=FALSE
                      ),
           caption = paste(gsub("_", " ",veQu[vq]), "by incentive group")
           ),
...
for(vq in 1:length(veQu)){
    regxtable <- tab1(exportFile[!(exportFile$e1a_inventivized %in% c("no", "yes")),veQu[vq]], graph=FALSE)$output.table
    capt <-  paste(gsub("_", " ",veQu[vq]), "for all non-incentivized folks")
    print(
        xtable(
            rename_NA(regxtable),
                 caption = capt
...
#SMS Push?
##########
cat(sprintf("\\section{SMS_Push}"), "\nHere is the data from the SMS Push section -- phase 2b")

pushCols <- grep(pattern="(^panel_round_[123]_reply)|(^pre_thermometer_round_[12]_reply)",
                  x=names(exportFile), value=FALSE)

##Currently doesn't work because it is junk
## for(wu in 1:length(pushCols)){
##     regxtable <- tab1(registered[, pushCols[wu]], graph=FALSE)$output.table
##     print(xtable(rename_NA(regxtable),caption = gsub("_", " ", pushCols[wu])),  table.placement="H")
## }


pushObs <- function(vector) {
    length(vector[!is.na(vector)])
    }


standardPrintOps <- function() {
    return('table.placement="H", include.rownames = FALSE, size = "footnotesize", rotate.colnames=TRUE')
    }
    


phase2bAnswers <- as.data.frame(apply(registered[ ,pushCols],2, pushObs))
names(phase2bAnswers) <- c("Valid Responses")
print(
    xtable(
        phase2bAnswers, caption="Number of valid answers for the Phase 2b questions"
        )
    )

#######
###Voting experience question
######
##########
cat(sprintf("\\section{Voter Experience Quiz}"), "\nHere is the data from the voter experience quiz ")

endlineCols <- grep(pattern="(^endlinesurvey_question_)",
                  x=names(registered), value=TRUE)

phase3bAnswers <- as.data.frame(apply(registered[ ,endlineCols],2, pushObs))
names(phase3bAnswers) <- c("Valid Responses")
print(
    xtable(
        phase3bAnswers, caption="Number of valid answers for the Phase 3b questions"
        )
    )

for(wu in 1:length(endlineCols)){
    regxtable <- tab1(registered[, endlineCols[wu]], graph=FALSE)$output.table
    print(xtable(rename_NA(regxtable),caption = gsub("_", " ", endlineCols[wu])),  table.placement="H")
}



## finalObs <- read.csv(file="/Volumes/Optibay-1TB/RSA_RCT/Monitor_Randomization/FINAL/for_praekelt_140503.csv", colClasses="character")
## table(finalObs$name[finalObs$citizen_monitor==1])

## tab1(exportFile$vip_question_4[exportFile$vip_question_3 %in% c("somewhat_likely", "very_likely") & !(exportFile$answerwin_question_age %in% c("15-19", "u14"))])
## tab1(exportFile$vip_question_4[exportFile$vip_question_3 %in% c("very_likely") &
##                                !(exportFile$answerwin_question_age %in% c("15-19", "u14")) &
##                                  exportFile$vip_question_2 %in% c("yes")
##                                  ],
##      bar.values="percent", missing=FALSE, main="Offical Unweighted VIP Predictions"
##                                )


## length(exportFile$panel_round_3_reply[!is.na(exportFile$panel_round_3_reply)])
## length(exportFile$panel_round_2_reply[!is.na(exportFile$panel_round_2_reply)])
## length(exportFile$panel_round_1_reply[!is.na(exportFile$panel_round_1_reply)])

## table(exportFile$pre_thermometer_round_2_reply)
