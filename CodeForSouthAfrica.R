...
vspath <- "/Volumes/Optibay-1TB/RSA_RCT/Data/VotingStations"

votingstations<-read.csv(file.path(vspath,"IEC - Voting station list - 2014 - Voting stations.csv"))
testdata<-read.csv(file.path(vspath, "TestData/Test_LookupDataset - Sheet1.csv"),colClasses="character")
...
#############
####This function takes a vector of prepared strings, where each one contains the appropriately formatted city+province info,
####calls the API and returns the result - Note that the ward result is not necessarily very trustworthy
####in that the town name often does not uniquely identify a ward
...
doAPICall<-function(sometown,pauseinterval=0)
{
    ...
    parsed <- unlist(lapply(res,fromJSON),recursive=FALSE,use.names=FALSE)
    ...
    return(rbind
           (parsed[,c("province","coords1","coords2","address","ward")])
           )
}
...
##  *Percentile* - Among All Stations within MaxDist of the Town given, a match must be in the X percentile of all matches.
##    That is, if Percentile = 0.10, then only the 10% of station names that are closest, in terms of Levenshtein Distance,
##    to the station name given will be considered for matching - Turns out that this should be set to a very large value
##  *MaxDist* - the maximum geographical distance to the matching station.  Should not be too small. 
##  *UncommonThresh* - extra "credit" is given for matches to include unusual words.
##     This threshold sets the number of occurrences in the corpus as a whole
##   (e.g., the number of polling station names containing the word) for a word to be considered unusual.
##
##The best match and confidence are selected by a score, that is a weighted sum of the Levenshtein Distance, Geographical Distance,
##Total Words Nearly Matched (e.g., extra credit is given for each word that is clo##sely matched), Total Unusual Words Matched,
##and Geographic Placement in the Same Town as the User Input.  The default weights here are nearly arbitrary - ideally they would be calibrated on data.
##Note that geog##raphic distance is squared, so its coefficient should remain relativey small


doEntry <- function(somerow,
                    CompDist=150,Percentile=0.65,MaxDist=40,UncommonThresh=200,LWeight=0.5,GeoWeight=0.2,MatchedWeight=35,UncommonWeight=50,TownWeight=15,
                    googleKey="AIzaSyBrdERRoZCaxMlqvE_CUZO-4ZrLQ3K22Mc"){
    ...
    if(is.na(APIres)){
            ...
            if(!is.na(APIres)){
                wrongProvince <- TRUE
                                        ...
                                        #If you still can't identify the town, just use the Province as a whole as the location.
                                        #In this case, the kilometer distance and town metrics are useful, so the weights on these are taken to zero.
    if (is.na(APIres))
        {
            ...
            provinceOnly <- TRUE
                                        ...
                                        #Sometimes the API returns multiple results - never more than two.
                                        #If these are close to one another, just use the first one - otherwise die (potentially undesirable behavior)
    if(nrow(APIres)>1)
	{
            if (abs(as.numeric(APIres[1,"coords1"])-as.numeric(APIres[2,"coords1"])<0.05) & (abs(as.numeric(APIres[1,"coords2"])-as.numeric(APIres[2,"coords2"])<0.05))) {
                    ...
                    return(NA)
                                        ...
                                        #Identify all Stations within 1 degree latitude and 1 degree longitude of the geolocated result (deliberately broad bounding box)
                                        #- This is done to draw a distribution of Levenshtein distances for comparison purposes
    ...
    dists <- spDistsN1(as.matrix(tocheck[,c("Longitude","Latitude")]),mypoint,longlat=TRUE)
    ...
    if(length(uncommonwords)!=0) {
        ...
        matcheduncommon <- rep(0.5,nrow(finalists))
    }
    ...
    if(!is.na(mytown)) {
        ...
        TownMatch <- rep(F,nrow(finalists))
    }
    
    TownMatch[is.na(TownMatch)] <- FALSE
